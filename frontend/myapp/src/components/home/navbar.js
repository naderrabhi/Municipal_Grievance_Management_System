...
export default function Header() {
  ...
  var dropdownLinks = [{ name: 'Se Connecter', link: '/login' }, { name: 'Inscription Employé', link: '/workersignup' }]
  ...
  if (value.exists === false) {
    dropdownLinks = [
      ...
      { name: 'Inscription Employé', link: '/workersignup' }
    ]
  }
  else if (value.exists === true && value.type === 'user') {
    dropdownLinks = [
      { name: 'Mes réclamations', link: '/usercomplaints' },
      { name: 'Déconnexion', link: '/' }
    ]
  }
  else if (value.exists === true && value.type === 'employee') {
    dropdownLinks = [
      { name: 'Mon tableau de bord', link: '/empdash' },
      { name: 'Déconnexion', link: '/' }
    ]
  }
  else if (value.exists === true && value.type === 'admin') {
    dropdownLinks = [
      { name: 'Tableau de bord Admin', link: '/admindash' },
      { name: 'Déconnexion', link: '/' }
    ]
  }
      ...
      <div className="container mx-auto p-2">
        <div className="flex flex-row justify-between items-center">
          <div className="flex flex-row justify-start items-center gap-2">
            ...
            <h1 className="text-white font-[Lobster] font-light text-lg">Municipalité de Mezzouna</h1>
          </div>
                ...
                <div className="flex flex-col justify-center items-center absolute text-white text-center px-4 hover:px-0 w-[200px] bg-gray-700 text-md rounded-md list-none gap-2">
                  {
                    dropdownLinks.map((data, index) =>
                      <li key={index} className="hover:bg-blue-700 hover:rounded-lg w-full hover:cursor-pointer py-1"
                        onClick={data.name === 'Déconnexion' ? () => { setValue({ exists: false }); setProfileImage(header_user); setDropdown(false); navigate('/') } : () => { navigate(data.link) }}>{data.name}</li>
                    )
                  }
                </div>
              }
